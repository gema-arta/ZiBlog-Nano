		...
		USART_Parity_TypeDef USART_Parity, USART_Mode_TypeDef USART_Mode)
{
	...
	USARTx->CR1 |= (uint8_t) ((uint8_t) USART_WordLength
	...
	USARTx->BRR2 = (uint8_t) ((BaudRate_Mantissa >> (uint8_t) 8)
			& (uint8_t) 0xF0);
		...
		USART_LastBit_TypeDef USART_LastBit)
{
	...
	USARTx->CR3
			&= (uint8_t) ~(USART_CR3_CPOL | USART_CR3_CPHA | USART_CR3_LBCL);
	/* Set the Clock Polarity, lock Phase, Last Bit Clock pulse */
	USARTx->CR3 |= (uint8_t) ((uint8_t) ((uint8_t) (USART_CPOL
			| (uint8_t) USART_CPHA) | USART_LastBit));
...
uint16_t USART_ReceiveData9(USART_TypeDef* USARTx)
{
	...
	temp = ((uint16_t) (((uint16_t) ((uint16_t) USARTx->CR1
			& (uint16_t) USART_CR1_R8)) << 1));
	return (uint16_t) (((uint16_t) ((uint16_t) USARTx->DR) | temp)
			& ((uint16_t) 0x01FF));
}
		...
		FunctionalState NewState)
{
	...
	itpos = (uint8_t) ((uint8_t) 1 << (uint8_t) ((uint8_t) USART_IT
			& (uint8_t) 0x0F));
...
void USART_ClearITPendingBit(USART_TypeDef* USARTx, USART_IT_TypeDef USART_IT)
{
	...
	bitpos
			= (uint8_t) ((uint8_t) ((uint8_t) USART_IT & (uint8_t) 0xF0)
					>> 0x04);
